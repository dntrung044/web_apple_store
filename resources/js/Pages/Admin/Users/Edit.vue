...
export default {
    ...
    methods: {
        deleteUser() {
            ...
            this.deleteAlertUserText = `Xoá người dùng ?`;
            setTimeout(() => (this.deleteAlertUser = false), 8000);
        },
        deleteUserConfirm() {
            router.delete(`/dashboard/users/${this.user.id}`);
        },
        updateUser() {
            if (this.avatar) {
            ...
            router.post(`/dashboard/users/${this.user.id}`, this.userInfo, {
                preserveState: false,
                preserveScroll: true,
